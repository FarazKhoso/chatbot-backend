...
if not api_key:
    ...
    api_key = None  # Will cause error when trying to use the model
...
def chat_stream():
    try:
        ...
        return jsonify({'error': f'Failed to process the streaming request: {str(e)}'}), 500
...
# For Vercel serverless deployment - Vercel will look for an application object
application = app
